...
abstract class WidgetInstance<T extends InstanceWidget> with NodeWithDepth implements Comparable<WidgetInstance> {
  ...
  final Queue<VoidCallback> _widgetUpdateCallbacks = DoubleLinkedQueue();

  /// Schedule [callback] to be invoked when the configuration
  /// [widget] of this instance is updated by the framework
  ///
  /// This mechanism is primarily used by visitor widgets like
  /// [Flexible] and [HitTestOccluder] which don't introduce any
  /// instances of their own and instead modify flags or parent
  /// data on an instance generated by their child [InstanceWidget]
  void scheduleWidgetUpdateCallback(VoidCallback callback) => _widgetUpdateCallbacks.addLast(callback);

  T _widget;
  T get widget => _widget;
  set widget(T value) {
    _widget = value;
    while (_widgetUpdateCallbacks.isNotEmpty) {
      _widgetUpdateCallbacks.removeFirst()();
    }
  }
